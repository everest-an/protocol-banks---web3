 ...
 * - `nonce` (`uint256`): Anti-replay parameter (see 鈥淪emi-abstracted Nonce Support鈥?)
...
interface IAccount {
     ...
     * * MUST pay the entryPoint (caller) at least the 鈥渕issingAccountFunds鈥?(which might
     *   be zero, in case the current account鈥檚 deposit is high enough)
     *
     * Returns an encoded packed validation data that is composed of the following elements:
     *
     * - `authorizer` (`address`): 0 for success, 1 for failure, otherwise the address of an authorizer contract
     * - `validUntil` (`uint48`): The UserOp is valid only up to this time. Zero for 鈥渋nfinite鈥?
