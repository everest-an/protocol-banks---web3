 ...
 *
 * Inspired by https://github.com/adria0/SolRsaVerify/blob/79c6182cabb9102ea69d4a2e996816091d5f1cd1[Adri脿 Massanet's work] (GNU General Public License v3.0).
 *
    ...
    function pkcs1Sha256(bytes32 digest, bytes memory s, bytes memory e, bytes memory n) internal view returns (bool) {
        unchecked {
            ...
            for (uint256 i = 0; i < length; i += 0x20) {
                ...
                if (sp < np) {
                    // s < n in the upper bits (everything before is equal) 鈫?s < n globally: ok
                    break;
                } else if (sp > np || p == length - 0x20) {
                    // s > n in the upper bits (everything before is equal) 鈫?s > n globally: fail
                    // or
                    // s = n and we are looking at the lower bits 鈫?s = n globally: fail
                    return false;
                }
            }
