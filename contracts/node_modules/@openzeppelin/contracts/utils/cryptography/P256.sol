...
library P256 {
    ...
    function recovery(bytes32 h, uint8 v, bytes32 r, bytes32 s) internal view returns (bytes32 x, bytes32 y) {
        ...
        uint256 ry2 = addmod(mulmod(addmod(mulmod(rx, rx, p), A, p), rx, p), B, p); // weierstrass equation y虏 = x鲁 + a.x + b
        uint256 ry = Math.modExp(ry2, P1DIV4, p); // This formula for sqrt work because P 鈮?3 (mod 4)
    ...
    function _jAdd(
        ...
        assembly ("memory-safe") {
            ...
            let zz1 := mulmod(z1, z1, p) // zz1 = z1虏
            let s1 := mulmod(mload(add(p1, 0x20)), mulmod(mulmod(z2, z2, p), z2, p), p) // s1 = y1*z2鲁
            let r := addmod(mulmod(y2, mulmod(zz1, z1, p), p), sub(p, s1), p) // r = s2-s1 = y2*z1鲁-s1 = y2*z1鲁-y1*z2鲁
            let u1 := mulmod(mload(p1), mulmod(z2, z2, p), p) // u1 = x1*z2虏
            let h := addmod(mulmod(x2, zz1, p), sub(p, u1), p) // h = u2-u1 = x2*z1虏-u1 = x2*z1虏-x1*z2虏

            // detect edge cases where inputs are identical
            switch and(iszero(r), iszero(h))
            // case 0: points are different
            case 0 {
                let hh := mulmod(h, h, p) // h虏
            ...
            case 1 {
                ...
                let yy := mulmod(y, y, p)
                let zz := mulmod(z, z, p)
                let m := addmod(mulmod(3, mulmod(x, x, p), p), mulmod(A, mulmod(zz, zz, p), p), p) // m = 3*x虏+a*z鈦?                let s := mulmod(4, mulmod(x, yy, p), p) // s = 4*x*y虏

                // x' = t = m虏-2*s
                rx := addmod(mulmod(m, m, p), sub(p, mulmod(2, s, p)), p)

                // y' = m*(s-t)-8*y鈦?= m*(s-x')-8*y鈦?                // cut the computation to avoid stack too deep
                let rytmp1 := sub(p, mulmod(8, mulmod(yy, yy, p), p)) // -8*y鈦?                let rytmp2 := addmod(s, sub(p, rx), p) // s-x'
                ry := addmod(mulmod(m, rytmp2, p), rytmp1, p) // m*(s-x')-8*y鈦?
                // z' = 2*y*z
    ...
    function _jDouble(uint256 x, uint256 y, uint256 z) private pure returns (uint256 rx, uint256 ry, uint256 rz) {
        assembly ("memory-safe") {
            ...
            // x' = t = m虏-2*s
            rx := addmod(mulmod(m, m, p), sub(p, mulmod(2, s, p)), p)
            // y' = m*(s-t)-8*y鈦?= m*(s-x')-8*y鈦?            ry := addmod(mulmod(m, addmod(s, sub(p, rx), p), p), sub(p, mulmod(8, mulmod(yy, yy, p), p)), p)
            // z' = 2*y*z
            rz := mulmod(2, mulmod(y, z, p), p)
        }
    }
    ...
    /**
     * @dev Compute G路u1 + P路u2 using the precomputed points for G and P (see {_preComputeJacobianPoints}).
     *
     ...
     *
     * 鈹屸攢鈹€鈹€鈹€鈹攢鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹?     * 鈹? i 鈹? 0    1     2     3 鈹?     * 鈹溾攢鈹€鈹€鈹€鈹尖攢鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹?     * 鈹? 0 鈹? 0    p    2p    3p 鈹?     * 鈹? 4 鈹? g  g+p  g+2p  g+3p 鈹?     * 鈹? 8 鈹?2g 2g+p 2g+2p 2g+3p 鈹?     * 鈹?12 鈹?3g 3g+p 3g+2p 3g+3p 鈹?     * 鈹斺攢鈹€鈹€鈹€鈹粹攢鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹?     *
     * Note that `_jAdd` (and thus `_jAddPoint`) does not handle the case where one of the inputs is a point at
     * infinity (z = 0). However, we know that since `N 鈮?1 mod 2` and `N 鈮?1 mod 3`, there is no point P such that
