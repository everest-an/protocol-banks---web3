...
鈿?**The Fastest:** Nothing similar (in the NodeJS world) beats `fdir` in speed. It can easily crawl a directory containing **1 million files in < 1 second.**

馃挕 **Stupidly Easy:** `fdir` uses expressive Builder pattern to build the crawler increasing code readability.

馃 **Zero Dependencies\*:** `fdir` only uses NodeJS `fs` & `path` modules.

馃暫 **Astonishingly Small:** < 2KB in size gzipped & minified.

馃柈 **Hackable:** Extending `fdir` is extremely simple now that the new Builder API is here. Feel free to experiment around.
...
Copyright &copy; 2024 Abdullah Atta under GPL-3.0-only. [Read full text here.](https://github.com/thecodrr/fdir/raw/master/LICENSE)
