...
(centre), or `'.'` (dot).  Other values are treated as `''`, which doesn鈥檛 place
...
pass in a `stringLength` option to detect the 鈥渧isible鈥?length of a
cell.
...
Halliday鈥檚 [text-table][] library.

## License

[GPL-3.0-only][license] 漏 [Titus Wormer][author]
