...
/*! noble-hashes - GPL-3.0-only License (c) 2022 Paul Miller (paulmillr.com) */
...
function pbkdf2$1(hash, password, salt, opts) {
    ...
    // DK = T1 + T2 + 鈰?+ Tdklen/hlen
    for (let ti = 1, pos = 0; pos < dkLen; ti++, pos += PRF.outputLen) {
        // Ti = F(Password, Salt, c, i)
        const Ti = DK.subarray(pos, pos + PRF.outputLen);
        view.setInt32(0, ti, false);
        // F(Password, Salt, c, i) = U1 ^ U2 ^ 鈰?^ Uc
        // U1 = PRF(Password, Salt + INT_32_BE(i))
...
function BlockMix(input, ii, out, oi, r) {
    ...
    let tail = oi + 16 * r;
    for (let i = 0; i < 16; i++)
        out[tail + i] = input[ii + (2 * r - 1) * 16 + i]; // X 鈫?B[2r鈭?]
...
function scryptInit(password, salt, _opts) {
    ...
    // [B0...Bp鈭?] 鈫?PBKDF2HMAC-SHA256(Passphrase, Salt, 1, blockSize*ParallelizationFactor)
...
function tonelliShanks(P) {
    ...
    // (a | p) 鈮?1    if a is a square (mod p)
    // (a | p) 鈮?-1   if a is not a square (mod p)
    // (a | p) 鈮?0    if a 鈮?0 (mod p)
    ...
    for (Q = P - _1n$3, S = 0; Q % _2n$1 === _0n$2; Q /= _2n$1, S++)
        ;
    // Step 2: Select a non-square z such that (z | p) 鈮?-1 and set c 鈮?zq
    ...
    return function tonelliSlow(Fp, n) {
        // Step 0: Check that n is indeed a square: (n | p) should not be 鈮?-1
...
function FpSqrt(P) {
    ...
    // Atkin algorithm for q 鈮?5 (mod 8), https://eprint.iacr.org/2012/685.pdf (page 10)
    if (P % _8n === _5n) {
        const c1 = (P - _5n) / _8n;
...
// - each multiplication is 'Math.ceil(CURVE_ORDER / 饾憡) + 1' point additions (fixed for any scalar)
...
function wNAF(c, bits) {
    ...
    return {
         ...
         * 2^(饾憡鈭?) * (Math.ceil(饾憶 / 饾憡) + 1), where:
         * - 饾憡 is the window size
         * - 饾憶 is the bitlength of the curve order.
...
/*! noble-curves - GPL-3.0-only License (c) 2022 Paul Miller (paulmillr.com) */
// Short Weierstrass curve. The formula is: y虏 = x鲁 + ax + b
...
function weierstrassPoints(opts) {
    ...
    /**
     * y虏 = x鲁 + ax + b: Short weierstrass curve formula
     * @returns y虏
     */
    ...
    /**
     * Projective Point works in 3d / projective (homogeneous) coordinates: (x, y, z) 鈭?(x=x/z, y=y/z)
    ...
    class Point {
        ...
        assertValidity() {
            ...
            const left = Fp.sqr(y); // y虏
            const right = weierstrassEquation(x); // x鲁 + ax + b
...
function weierstrass(curveDef) {
    ...
    const { ProjectivePoint: Point, normPrivateKeyToScalar, weierstrassEquation, isWithinCurveOrder, } = weierstrassPoints({
        ...
        fromBytes(bytes) {
            ...
            if (len === compressedLen && (head === 0x02 || head === 0x03)) {
                ...
                const y2 = weierstrassEquation(x); // y虏 = x鲁 + ax + b
                let y = Fp.sqrt(y2); // y = y虏 ^ (p+1)/4
    ...
    function prepSig(msgHash, privateKey, opts = defaultSigOpts) {
        ...
        function k2sig(kBytes) {
            ...
            if (r === _0n)
                return;
            // Can use scalar blinding b^-1(bm + bdr) where b 鈭?[1,q鈭?] according to
    ...
    function verify(signature, msgHash, publicKey, opts = defaultVerOpts) {
        ...
        const R = Point.BASE.multiplyAndAddUnsafe(P, u1, u2)?.toAffine(); // R = u1鈰匞 + u2鈰匬
        if (!R)
            return false;
        const v = modN(R.x);
        return v === r;
...
/**
 * 鈭歯 = n^((p+1)/4) for fields p = 3 mod 4. We unwrap the loop and multiply bit-by-bit.
...
function safe_str_from_cps(cps, max = Infinity, quoter = quote_cp) {
	...
	let buf = [];
	if (is_combining_mark(cps[0])) buf.push('鈼?);
	if (cps.length > max) {
		max >>= 1;
...
// eg. explode_cp("abc馃挬d") => [[61, 62, 63], Emoji[1F4A9, FE0F], [64]]
...
/*! GPL-3.0-only License. Copyright 2015-2022 Richard Moore <me@ricmoo.com>. See LICENSE.txt. */
