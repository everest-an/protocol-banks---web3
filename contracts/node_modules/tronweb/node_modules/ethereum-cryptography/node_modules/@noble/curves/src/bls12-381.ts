/*! noble-curves - GPL-3.0-only License (c) 2022 Paul Miller (paulmillr.com) */
...
// G2 is the order-q subgroup of E2(Fp虏) : y虏 = x鲁+4(1+鈭氣垝1),
// where Fp2 is Fp[鈭氣垝1]/(x2+1). #E2(Fp2 ) = h2q, where
// G虏 - 1
// h2q
...
const Fp2: mod.IField<Fp2> & Fp2Utils = {
  ...
  inv: ({ c0: a, c1: b }) => {
    ...
    // (a + bu)(a - bu) = a虏 + b虏
    //
    // which holds because u虏 = -1. This can be rewritten as
    //
    // (a + bu)(a - bu)/(a虏 + b虏) = 1
    //
    // because a虏 + b虏 = 0 has no nonzero solutions for (a, b).
    // This gives that (a - bu)/(a虏 + b虏) is the inverse
...
const Fp6Square = ({ c0, c1, c2 }: Fp6) => {
  let t0 = Fp2.sqr(c0); // c0虏
  let t1 = Fp2.mul(Fp2.mul(c0, c1), _2n); // 2 * c0 * c1
  let t3 = Fp2.mul(Fp2.mul(c1, c2), _2n); // 2 * c1 * c2
  let t4 = Fp2.sqr(c2); // c2虏
  return {
    c0: Fp2.add(Fp2.mulByNonresidue(t3), t0), // T3 * (u + 1) + T0
    c1: Fp2.add(Fp2.mulByNonresidue(t4), t1), // T4 * (u + 1) + T1
    // T1 + (c0 - c1 + c2)虏 + T3 - T0 - T4
...
const Fp6: mod.IField<Fp6> & Fp6Utils = {
  ...
  inv: ({ c0, c1, c2 }) => {
    let t0 = Fp2.sub(Fp2.sqr(c0), Fp2.mulByNonresidue(Fp2.mul(c2, c1))); // c0虏 - c2 * c1 * (u + 1)
    let t1 = Fp2.sub(Fp2.mulByNonresidue(Fp2.sqr(c2)), Fp2.mul(c0, c1)); // c2虏 * (u + 1) - c0 * c1
    let t2 = Fp2.sub(Fp2.sqr(c1), Fp2.mul(c0, c2)); // c1虏 - c0 * c2
...
// Finite extension field over irreducible polynominal.
// Fp鈧佲倐 = Fp鈧喡?=> Fp鈧偮?// Fp鈧?w) / (w虏 - 纬) where 纬 = v
...
function Fp4Square(a: Fp2, b: Fp2): { first: Fp2; second: Fp2 } {
  ...
  const b2 = Fp2.sqr(b);
  return {
    first: Fp2.add(Fp2.mulByNonresidue(b2), a2), // b虏 * Nonresidue + a虏
    second: Fp2.sub(Fp2.sub(Fp2.sqr(Fp2.add(a, b)), a2), b2), // (a + b)虏 - a虏 - b虏
  };
}
type Fp12Utils = {
...
const Fp12: mod.IField<Fp12> & Fp12Utils = {
  ...
  inv: ({ c0, c1 }) => {
    let t = Fp6.inv(Fp6.sub(Fp6.sqr(c0), Fp6.mulByNonresidue(Fp6.sqr(c1)))); // 1 / (c0虏 - c1虏 * v)
  ...
  finalExponentiate: (num) => {
    ...
    // (t2 * t5)^(q虏) * (t4 * t1)^(q鲁) * (t6 * t1.conj)^(q^1) * t7 * t3.conj * t1
...
// SWU Map - Fp2 to G2': y虏 = x鲁 + 240i * x + 1012 + 1012i
...
// Fp鈧佲倐 = Fp鈧喡?=> Fp鈧偮?// Fp(u) / (u虏 - 尾) where 尾 = -1
// Fp鈧?v) / (v鲁 - 尉) where 尉 = u + 1
// Fp鈧?w) / (w虏 - 纬) where 纬 = v
...
export const bls12_381: CurveFn<Fp, Fp2, Fp6, Fp12> = bls({
  ...
  // G1 is the order-q subgroup of E1(Fp) : y虏 = x鲁 + 4, #E1(Fp) = h1q, where
  // characteristic; z + (z鈦?- z虏 + 1)(z - 1)虏/3
  G1: {
    Fp,
    // cofactor; (z - 1)虏/3
    ...
    isTorsionFree: (c, point): boolean => {
      ...
      // const Q = S.double();
      // const S2 = S.sigma();
      // // [(z虏 鈭?1)/3](2蟽(P) 鈭?P 鈭?蟽虏(P)) 鈭?蟽虏(P) = O
    ...
    fromBytes: (bytes: Uint8Array): AffinePoint<Fp> => {
      ...
      if (value.length === 48 && compressed) {
        ...
        const right = Fp.add(Fp.pow(x, _3n), Fp.create(bls12_381.params.G1b)); // y虏 = x鲁 + b
        let y = Fp.sqrt(right);
  ...
  G2: {
    ...
    isTorsionFree: (c, P): boolean => {
      return P.multiplyUnsafe(bls12_381.params.x).negate().equals(G2psi(c, P)); // 蠄(P) == [u](P)
      // Older version: https://eprint.iacr.org/2019/814.pdf
      // 唯虏(P) => 唯鲁(P) => [z]唯鲁(P) where z = -x => [z]唯鲁(P) - 唯虏(P) + P == O
    ...
    clearCofactor: (c, P) => {
      ...
      let t2 = G2psi(c, P);                   // 唯(P)
      let t3 = P.double();                    // 2P
      t3 = G2psi2(c, t3);                     // 唯虏(2P)
      t3 = t3.subtract(t2);                   // 唯虏(2P) - 唯(P)
      t2 = t1.add(t2);                        // [-x]P + 唯(P)
      t2 = t2.multiplyUnsafe(x).negate();     // [x虏]P - [x]唯(P)
      t3 = t3.add(t2);                        // 唯虏(2P) - 唯(P) + [x虏]P - [x]唯(P)
      t3 = t3.subtract(t1);                   // 唯虏(2P) - 唯(P) + [x虏]P - [x]唯(P) + [x]P
      const Q = t3.subtract(P);               // 唯虏(2P) - 唯(P) + [x虏]P - [x]唯(P) + [x]P - 1P
      return Q;                               // [x虏-x-1]P + [x-1]唯(P) + 唯虏(2P)
    },
    fromBytes: (bytes: Uint8Array): AffinePoint<Fp2> => {
      ...
      if (value.length === 96 && compressed) {
        ...
        const right = Fp2.add(Fp2.pow(x, _3n), b); // y虏 = x鲁 + 4 * (u+1) = x鲁 + b
    ...
    Signature: {
      ...
      fromHex(hex: Hex): ProjPointType<Fp2> {
        ...
        const y2 = Fp2.add(Fp2.pow(x, _3n), bls12_381.params.G2b); // y虏 = x鲁 + 4
        // The slow part
        let y = Fp2.sqrt(y2);
  ...
  params: {
    ...
    r: Fr.ORDER, // order; z鈦?鈭?z虏 + 1; CURVE.n from other curves
  },
  htfDefaults,
  hash: sha256,
  randomBytes,
