...
function uvRatio(u, v) {
    const P = ED25519_P;
    const v3 = (0, modular_js_1.mod)(v * v * v, P); // v鲁
    const v7 = (0, modular_js_1.mod)(v3 * v3 * v, P); // v鈦?    // (p+3)/8 and (p-5)/8
    const pow = ed25519_pow_2_252_3(u * v7).pow_p_5_8;
    let x = (0, modular_js_1.mod)(u * v3 * pow, P); // (uv鲁)(uv鈦?^(p-5)/8
    const vx2 = (0, modular_js_1.mod)(v * x * x, P); // vx虏
    const root1 = x; // First root candidate
    const root2 = (0, modular_js_1.mod)(x * ED25519_SQRT_M1, P); // Second root candidate
    const useRoot1 = vx2 === u; // If vx虏 = u (mod p), x is a square root
    const useRoot2 = vx2 === (0, modular_js_1.mod)(-u, P); // If vx虏 = -u, set x <-- x * 2^((p-1)/4)
    const noRoot = vx2 === (0, modular_js_1.mod)(-u * ED25519_SQRT_M1, P); // There is no valid root, vx虏 = -u鈭?-1)
    if (useRoot1)
        x = root1;
    if (useRoot2 || noRoot)
...
const ed25519Defaults = /* @__PURE__ */ (() => ({
    ...
    // Finite field 饾斀p over which we'll do calculations; 2n**255n - 19n
    Fp,
