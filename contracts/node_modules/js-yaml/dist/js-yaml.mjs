...
function isPlainSafeFirst(c) {
  ...
  return isPrintable(c) && c !== CHAR_BOM
    ...
    // - (c-indicator ::=
    // 鈥?鈥?| 鈥?鈥?| 鈥?鈥?| 鈥?鈥?| 鈥淸鈥?| 鈥淽鈥?| 鈥渰鈥?| 鈥渳鈥?    && c !== CHAR_MINUS
    && c !== CHAR_QUESTION
    && c !== CHAR_COLON
    && c !== CHAR_COMMA
    && c !== CHAR_LEFT_SQUARE_BRACKET
    ...
    && c !== CHAR_LEFT_CURLY_BRACKET
    && c !== CHAR_RIGHT_CURLY_BRACKET
    // | 鈥?鈥?| 鈥?鈥?| 鈥?鈥?| 鈥?鈥?| 鈥渱鈥?| 鈥?鈥?| 鈥?鈥?| 鈥?鈥?| 鈥?鈥?    && c !== CHAR_SHARP
    && c !== CHAR_AMPERSAND
    && c !== CHAR_ASTERISK
    && c !== CHAR_EXCLAMATION
...
function foldString(string, width) {
  // In folded style, $k$ consecutive newlines output as $k+1$ newlines鈥?  // unless they're before or after a more-indented line, or at the very
...
function writeNode(state, level, object, block, compact, iskey, isblockseq) {
  ...
  if (duplicate && state.usedDuplicates[duplicateIndex]) {
    ...
    if (state.tag !== null && state.tag !== '?') {
      ...
      // [38] ns-word-char    ::=  ns-dec-digit | ns-ascii-letter | 鈥?鈥?      // [39] ns-uri-char     ::=  鈥?鈥?ns-hex-digit ns-hex-digit | ns-word-char | 鈥?鈥?      //                         | 鈥?鈥?| 鈥?鈥?| 鈥?鈥?| 鈥?鈥?| 鈥淍鈥?| 鈥?鈥?| 鈥?鈥?| 鈥?鈥?| 鈥?鈥?| 鈥?鈥?      //                         | 鈥淿鈥?| 鈥?鈥?| 鈥?鈥?| 鈥渵鈥?| 鈥?鈥?| 鈥?鈥?| 鈥?鈥?| 鈥?鈥?| 鈥淸鈥?| 鈥淽鈥?      //
