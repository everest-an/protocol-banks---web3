...
- 馃敁 Secure: audited [noble](https://paulmillr.com/noble/) cryptography, no network code, [hedged signatures](#transactions-create-sign)
- 馃敾 Tree-shakeable: unused code is excluded from your builds
- 馃攳 Reliable: 150MB of test vectors from EIPs, ethers and viem
- 鉁嶏笍 Core: transactions, addresses, messages
- 馃實 Network-related: execute Uniswap & Chainlink, fetch tx history
- 馃 Advanced: type-safe ABI parsing, RLP, SSZ, KZG, Verkle
- 馃 29KB gzipped (1300 lines) for core, just 3 deps
...
- We're not able to handle contracts with method overload (same function names with different args) 鈥?the code will still work, but not types
