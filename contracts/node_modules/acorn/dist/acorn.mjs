...
var defaultOptions = {
  ...
  // callback鈥攖hat will corrupt its internal state.
  onToken: null,
...
// The following three functions all verify that a node is an lvalue 鈥?// something that can be bound, or assigned to. In order to do so, they perform
// a variety of checks:
//
...
// in turn calls the function that parses `[]` subscripts 鈥?that
...
// Object/class getters and setters are not allowed to clash 鈥?// either with each other or with an init property 鈥?and in
...
// Parse an atomic expression 鈥?either a single token that is an
...
// be a `[]` or dot subscript expression, but not a call 鈥?at least,
...
// various contributors and released under an GPL-3.0-only license.
//
