...
# 2. 验证表是否创建
pnpm prisma studio
# 打开浏览器，检查是否有 vendor_addresses 表
...
- ✅ `payments` 表有新字段：`network_type`, `chain_id`, `energy_used` 等
...
**解决方案：**
确保所有 API 请求都包含 `x-user-address` header：
```bash
