# Protocol Banks — Security Audit Report
...
Exposed secrets:
- `DEPLOYER_PRIVATE_KEY` — 64-char hex EVM private key (controls all deployed contracts)
- `DEPLOYER_MNEMONIC` — 12-word BIP39 mnemonic (derives unlimited accounts)
- `REDIS_URL` — Full connection string with password to cloud Redis
- `RANGO_API_KEY` — DEX swap API key
- `DATABASE_URL` — Base64-encoded PostgreSQL credentials (`postgres:postgres`, SSL disabled)

**Remediation:**
1. **TODAY:** Rotate ALL exposed credentials (Redis, Rango, DB password)
2. **TODAY:** Assume deployer key/mnemonic compromised — migrate contracts & funds to new wallet
...
### C-02: Admin panel — client-side only authentication
...
### C-03: Audit log GET — no authentication, no user isolation
...
```
GET /api/audit-log?actor=0x[any_address]  →  Returns complete activity history
```
...
### C-04: Accounting reports — header-only auth (spoofable)
...
### C-05: X.402 verify — unauthenticated + no on-chain verification

**File:** `app/api/x402/verify/route.ts` lines 24-132
**Impact:** Payment fraud — anyone can mark any payment as verified with a fake txHash
...
Response distinguishes "payload tampered" vs "secret incorrect" — enables secret enumeration.

**Remediation:** Return uniform error message regardless of failure reason.

---

### C-08: `getAuthenticatedAddress()` — no signature verification

**File:** `lib/api-auth.ts` lines 23-37
**Impact:** Identity spoofing — any client can impersonate any wallet
...
### H-06: Batch payment recipients — no type validation
...
### H-07: File upload — no type validation
...
| M-04 | MCP subscriptions — no Zod validation | `app/api/mcp-subscriptions/route.ts:40` | 8 unvalidated fields directly to Prisma |
...
CRITICAL:
  jspdf <=4.0.0       — Path traversal + PDF injection + DoS

HIGH:
  next 10.0.0-15.5.9   — Image Optimizer DoS + RSC deserialization DoS
