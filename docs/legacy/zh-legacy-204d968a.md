# 多网络支�?- 故障排查指南

## 📋 目录

1. [数据库连接问题](#数据库连接问�?
2. [迁移失败](#迁移失败)
3. [API 错误](#api-错误)
4. [地址验证问题](#地址验证问题)
5. [UI 组件问题](#ui-组件问题)
6. [性能问题](#性能问题)
7. [网络检测问题](#网络检测问�?

---

## 数据库连接问�?

### 问题 1: 无法连接到数据库服务�?

**症状�?*
```
Error: P1001
Can't reach database server at localhost:51214
```

**可能原因�?*
1. PostgreSQL 服务未启�?
2. 端口配置错误
3. 防火墙阻止连�?

**解决步骤�?*

```bash
# 1. 检�?PostgreSQL 是否运行
# Windows
tasklist | findstr postgres
sc query postgresql-x64-15

# 2. 检查端口是否监�?
netstat -an | findstr :51214

# 3. 尝试手动连接
psql -h localhost -p 51214 -U postgres

# 4. 如果使用 Docker
docker ps | findstr postgres
docker logs <container-id>

# 5. 检�?.env 配置
type .env | findstr DATABASE_URL
```

**修复方法�?*

```bash
# 启动 PostgreSQL 服务
# Windows
net start postgresql-x64-15

# Docker
docker start <postgres-container>

# 验证连接
psql -h localhost -p 51214 -U postgres -c "SELECT version();"
```

### 问题 2: 认证失败

**症状�?*
```
FATAL: password authentication failed for user "postgres"
```

**解决方法�?*

```bash
# 1. 检�?DATABASE_URL 中的密码
# .env 文件中：
DATABASE_URL="postgresql://postgres:YOUR_PASSWORD@localhost:51214/..."

# 2. 重置 PostgreSQL 密码（如果需要）
psql -U postgres
ALTER USER postgres PASSWORD 'new_password';

# 3. 更新 .env 文件中的密码
```

---

## 迁移失败

### 问题 3: Schema 推送失�?

**症状�?*
```
Error: P1012
Schema validation error
```

**可能原因�?*
- Schema 语法错误
- 数据库权限不�?
- 现有数据与新 schema 冲突

**解决步骤�?*

```bash
# 1. 验证 Prisma schema
pnpm prisma validate

# 2. 查看详细错误
pnpm prisma db push --help

# 3. 强制重置（⚠�?会删除数据）
pnpm prisma db push --force-reset --accept-data-loss

# 4. 检查数据库权限
psql -U postgres -c "SHOW is_superuser;"
```

### 问题 4: 唯一约束冲突

**症状�?*
```
Error: Unique constraint failed on the fields: (`vendor_id`,`network`)
```

**原因�?* 同一个供应商在同一网络上已有地址

**解决方法�?*

```sql
-- 查找重复记录
SELECT vendor_id, network, COUNT(*)
FROM vendor_addresses
GROUP BY vendor_id, network
HAVING COUNT(*) > 1;

-- 删除重复记录（保留最新的�?
DELETE FROM vendor_addresses a
USING vendor_addresses b
WHERE a.vendor_id = b.vendor_id
  AND a.network = b.network
  AND a.created_at < b.created_at;
```

### 问题 5: SQL 迁移脚本执行失败

**症状�?*
```
ERROR: function ensure_one_primary_address_per_network() does not exist
```

**原因�?* 使用 `prisma db push` 不会创建函数/触发�?

**解决方法�?*

```bash
# 手动运行 SQL 迁移
psql -h localhost -p 51214 -U postgres -d template1 -f scripts\009_multi_network_support.sql

# 或逐个执行（如果上面失败）
psql -h localhost -p 51214 -U postgres -d template1

# 然后�?psql 中：
\i scripts/009_multi_network_support.sql
```

---

## API 错误

### 问题 6: 401 Unauthorized

**症状�?*
```json
{ "error": "Unauthorized" }
```

**原因�?* 缺少认证�?

**解决方法�?*

```bash
# 确保所有请求都包含 x-wallet-address
curl http://localhost:3000/api/vendors/multi-network \
  -H "x-wallet-address: 0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb2"

# 检�?API 路由代码
# 所有路由都应该有：
const authAddress = await getAuthenticatedAddress(request)
if (!authAddress) {
  return NextResponse.json({ error: "Unauthorized" }, { status: 401 })
}
```

### 问题 7: 403 Forbidden

**症状�?*
```json
{ "error": "Forbidden: Access denied to other wallets" }
```

**原因�?* 尝试访问其他用户的数�?

**解决方法�?*

1. 确保请求自己的数�?
2. 检�?RLS 策略是否正确

```sql
-- 检�?RLS 是否启用
SELECT tablename, rowsecurity
FROM pg_tables
WHERE tablename = 'vendor_addresses';

-- 查看 RLS 策略
SELECT * FROM pg_policies WHERE tablename = 'vendor_addresses';
```

### 问题 8: 500 Internal Server Error

**症状�?*
```json
{ "error": "Internal Server Error" }
```

**调试步骤�?*

```bash
# 1. 查看服务器日�?
# 终端中会显示详细错误

# 2. 检�?Prisma client 是否最�?
pnpm prisma generate

# 3. 重启开发服务器
# Ctrl+C 停止
pnpm dev

# 4. 检查数据库连接
pnpm prisma studio
```

---

## 地址验证问题

### 问题 9: Invalid address 错误

**症状�?*
```json
{ "error": "Invalid address: Address must be 42 characters long" }
```

**常见原因和解决方法：**

```typescript
// �?错误：地址长度不对
"0x123"  // 太短

// �?正确�?2 字符（包�?0x�?
"0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb2"

// �?错误：TRON 地址格式错误
"T123"  // 太短

// �?正确�?4 字符，Base58
"TQn9Y2khEsLJW1ChVWFMSMeRDow5KcbLSE"

// �?错误：混淆网�?
// 不能�?EVM 地址当作 TRON 地址
{
  "network": "tron",
  "address": "0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb2"  // �?
}

// �?正确
{
  "network": "tron",
  "address": "TQn9Y2khEsLJW1ChVWFMSMeRDow5KcbLSE"  // �?
}
```

### 问题 10: 网络自动检测不准确

**症状�?* 所�?EVM 地址都被识别�?"ethereum"

**原因�?* 这是预期行为 - 无法从地址本身区分 EVM �?

**解决方法�?*

```typescript
// 用户需要手动指定具体的 EVM 网络
const address = {
  network: "base",  // 手动指定
  address: "0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb2",
  isPrimary: true
}

// 或在 UI 中提供网络选择�?
<NetworkSelector
  value={network}
  onChange={setNetwork}
  filterByType="EVM"  // 只显�?EVM 网络
/>
```

---

## UI 组件问题

### 问题 11: VendorAddressManager 组件不显�?

**可能原因�?*
1. 组件未正确导�?
2. Props 类型不匹�?
3. 缺少必需�?props

**解决方法�?*

```tsx
// 检查导入路�?
import { VendorAddressManager } from "@/components/vendors/vendor-address-manager"

// 确保传递所有必需�?props
<VendorAddressManager
  vendorId={vendor.id}              // �?必需
  addresses={vendor.addresses || []} // �?必需，确保是数组
  onUpdate={() => refetch()}         // �?必需
  userAddress={userAddress}          // �?必需
/>

// 检查类型定�?
interface VendorAddress {
  id: string
  network: string
  address: string
  label?: string
  isPrimary: boolean
  verifiedAt?: string
  createdAt: string
}
```

### 问题 12: 网络徽章颜色不正�?

**原因�?* Tailwind CSS 类未包含在构建中

**解决方法�?*

```typescript
// �?NetworkBadge 组件中使用完整的类名
// �?错误：动态类�?
const color = `bg-${colorName}-500`

// �?正确：完整类�?
const getNetworkColor = (network: string) => {
  if (network === "tron") return "bg-red-500 hover:bg-red-600"
  if (network === "ethereum") return "bg-blue-500 hover:bg-blue-600"
  // ...
}
```

### 问题 13: Dialog 不关�?

**症状�?* 点击保存后对话框仍然打开

**解决方法�?*

```typescript
const handleSave = async () => {
  try {
    await saveAddress()
    setIsDialogOpen(false)  // �?确保关闭对话�?
    onUpdate()               // �?触发刷新
  } catch (error) {
    // 错误时不关闭对话�?
    console.error(error)
  }
}
```

---

## 性能问题

### 问题 14: 查询速度�?

**症状�?* API 响应时间 > 2 �?

**诊断�?*

```typescript
// 启用 Prisma 查询日志
// prisma/schema.prisma
datasource db {
  provider = "postgresql"
  url      = env("DATABASE_URL")
}

generator client {
  provider = "prisma-client-js"
  log      = ["query", "info", "warn", "error"]
}
```

**优化方法�?*

```sql
-- 1. 检查索引是否存�?
SELECT * FROM pg_indexes WHERE tablename = 'payments';

-- 2. 创建缺失的索�?
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_payments_network_type
ON payments(network_type);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_payments_created_at
ON payments(created_at DESC);

-- 3. 分析查询性能
EXPLAIN ANALYZE
SELECT * FROM payments
WHERE network_type = 'TRON'
ORDER BY created_at DESC
LIMIT 50;

-- 4. 更新统计信息
ANALYZE payments;
ANALYZE vendor_addresses;
```

### 问题 15: 内存使用过高

**症状�?* Node.js 进程占用大量内存

**解决方法�?*

```typescript
// 1. 使用分页，避免一次性加载太多数�?
const payments = await prisma.payment.findMany({
  take: 100,  // �?限制返回数量
  skip: offset,
  where: { ... }
})

// 2. 只选择需要的字段
const payments = await prisma.payment.findMany({
  select: {
    id: true,
    amount: true,
    network_type: true,
    // 不要�?select: *
  }
})

// 3. 使用游标分页（大数据集）
const payments = await prisma.payment.findMany({
  take: 100,
  cursor: { id: lastId },
  orderBy: { created_at: 'desc' }
})
```

---

## 网络检测问�?

### 问题 16: TRON 地址被识别为无效

**症状�?*
```json
{ "error": "Invalid address: Invalid Base58 encoding" }
```

**可能原因�?*
1. 地址包含无效�?Base58 字符
2. 地址长度不正�?

**解决方法�?*

```typescript
// 验证 TRON 地址
import { validateAddress } from "@/lib/address-utils"

const validation = validateAddress("TQn9Y2khEsLJW1ChVWFMSMeRDow5KcbLSE")

if (!validation.isValid) {
  console.error(validation.error)
  // "Invalid Base58 encoding" 或其他错�?
}

// 检查地址格式
// �?正确：T 开头，34 字符，只包含 Base58 字符
"TQn9Y2khEsLJW1ChVWFMSMeRDow5KcbLSE"

// �?错误示例�?
"TQn9Y2khEs0LJW1ChVWFMSMeRDow5KcbLSE"  // 包含 '0'（不�?Base58�?
"TQn9Y2khEsLJW1ChVWFMSMeRDow"        // 太短
```

### 问题 17: 批量支付检测到错误的网�?

**症状�?* 批量支付被分配到错误的网�?

**原因�?* 只检测了第一个收款人地址

**解决方法�?*

```typescript
// 确保所有收款人在同一网络�?
const recipients = [
  { address: "0x...", amount: "10" },  // EVM
  { address: "0x...", amount: "20" },  // EVM
  // �?不要混合网络类型
  // { address: "TQn9...", amount: "30" }  // TRON
]

// 验证所有地址
recipients.forEach(r => {
  const validation = validateAddress(r.address)
  if (validation.type !== expectedType) {
    throw new Error("所有收款人必须在同一网络�?)
  }
})
```

---

## 日志和调�?

### 启用详细日志

```typescript
// lib/prisma.ts
const prisma = new PrismaClient({
  log: ['query', 'error', 'warn'],
})

// 或在开发环�?
const prisma = new PrismaClient({
  log: process.env.NODE_ENV === 'development'
    ? ['query', 'error', 'warn', 'info']
    : ['error'],
})
```

### 使用浏览器开发工�?

```javascript
// 在浏览器控制台中调试
// 1. 检查网络请�?
// DevTools �?Network �?筛�?Fetch/XHR

// 2. 查看请求详情
// 点击请求 �?Headers �?检�?x-wallet-address
// Payload �?检查请求体
// Response �?检查响�?

// 3. 检�?React 状�?
// React DevTools �?Components �?选择组件 �?查看 props/state
```

---

## 获取帮助

如果上述方法都无法解决问题：

1. **检查文�?*
   - [MIGRATION_GUIDE.md](./MIGRATION_GUIDE.md)
   - [TESTING_GUIDE.md](./TESTING_GUIDE.md)
   - [MULTI_NETWORK_IMPLEMENTATION.md](./MULTI_NETWORK_IMPLEMENTATION.md)

2. **查看日志**
   - 开发服务器控制台输�?
   - 浏览器控制台（F12�?
   - PostgreSQL 日志

3. **验证环境**
   ```bash
   node --version  # 需�?18+
   pnpm --version
   psql --version
   ```

4. **重新开�?*
   ```bash
   # 清理并重新安�?
   rm -rf node_modules .next
   pnpm install
   pnpm prisma generate
   pnpm dev
   ```

---

**最后更新：** 2026-02-07
