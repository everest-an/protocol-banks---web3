...
- ✅ Migrates existing vendor addresses to new table
- ✅ Updates existing payment records with network_type and chain_id
...
- Vendor with `wallet_address="0x123..."` and `chain="Ethereum"`
  → Creates VendorAddress with `network="ethereum"`, `address="0x123..."`, `is_primary=true`

### Existing Payments

The migration will:
1. Set `network_type` based on chain:
   - TRON chains → `network_type="TRON"`
   - All others → `network_type="EVM"`
2. Set `chain_id` for EVM networks:
   ...
   - Base → `chain_id=8453`
   - BSC → `chain_id=56`
3. Normalize chain names to lowercase (e.g., "Ethereum" → "ethereum")
