    ...
    <html>
      ...
      <body>
        <div class="page">
          ...
          ${(invoice.memo || invoice.category || invoice.group_name) ? `
          <div class="payment-info" style="margin-top: 16px;">
            <h3>Notes & Classification</h3>
            ${invoice.memo ? `
            <div class="payment-row">
              <span class="payment-label">Memo</span>
              <span class="payment-value" style="font-family: inherit; font-size: 14px;">${invoice.memo}</span>
            </div>` : ''}
            ${invoice.category ? `
            <div class="payment-row">
              <span class="payment-label">Purpose</span>
              <span class="payment-value" style="background: #f3f4f6; padding: 2px 8px; border-radius: 4px; font-family: inherit; font-size: 13px;">${invoice.category}</span>
            </div>` : ''}
            ${invoice.group_name ? `
            <div class="payment-row">
              <span class="payment-label">Payment Group</span>
              <span class="payment-value" style="font-family: inherit; font-size: 14px;">${invoice.group_name}</span>
            </div>` : ''}
          </div>` : ''}
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Enrich invoice with payment memo/category if a tx_hash is available
    if (invoice.tx_hash) {
      try {
        const payment = await prisma.payment.findFirst({
          where: { tx_hash: invoice.tx_hash },
          include: { group: true },
        })
        if (payment) {
          if (payment.memo && !invoice.memo) invoice.memo = payment.memo
          if (payment.category && !invoice.category) invoice.category = payment.category
          if (payment.group?.name && !invoice.group_name) invoice.group_name = payment.group.name
        }
      } catch {
