-- =====================================================
-- Protocol Banks - Fix Auth Tables Migration
-- Run this to ensure all auth tables are properly configured
-- =====================================================

-- Drop existing policies first to avoid conflicts
DROP POLICY IF EXISTS "Users can view their own data" ON auth_users;
DROP POLICY IF EXISTS "Users can update their own data" ON auth_users;
DROP POLICY IF EXISTS "System can insert users" ON auth_users;
DROP POLICY IF EXISTS "System can manage magic links" ON magic_links;
...
DROP POLICY IF EXISTS "System can manage sessions" ON auth_sessions;
DROP POLICY IF EXISTS "System can manage recovery requests" ON wallet_recovery_requests;
DROP POLICY IF EXISTS "Users can manage their device shares" ON device_shares;
...
-- 3. Ensure embedded_wallets table exists
...
-- 4. Ensure auth_sessions table exists
...
-- 5. Ensure wallet_recovery_requests table exists
...
-- 6. Ensure device_shares table exists
...
BEGIN
  ...
  RAISE NOTICE 'Tables: auth_users, magic_links, embedded_wallets, auth_sessions, wallet_recovery_requests, device_shares';
END $$;
